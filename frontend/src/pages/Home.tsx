...
// import Deconnexion from "../components/log/logout/Deconnexion";
...
//         <nav className="h-full bg-gradient-to-b from-blue-900 to-blue-800 shadow-2xl border-r border-blue-700">
//           <div className="flex justify-center items-center p-6 border-b border-blue-700">
//             <img className="w-32 filter brightness-125 drop-shadow-lg" src="/images/Logo_WS.png" alt="Logo" />
//           </div>
...
//                     }`}
//                 >
//                   <Icon size={28} className={activePage === item.id ? "text-white" : "text-blue-300"} />
//                   <span className="text-xl font-semibold">{item.label}</span>
//                   {activePage === item.id && (
//                     <div className="ml-auto w-2 h-2 bg-white rounded-full" />
//                   )}
...
//                   >
//                     <div className="p-4 border-b border-gray-200">
//                       <p className="text-gray-800 font-semibold">{user?.name || "Admin"}</p>
...
//                       <button
//                         onClick={handleLogout}
//                         className="flex items-center gap-3 w-full p-3 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
//                       >
//                         <LogOut size={18} />
//                         <span>Déconnexion</span>
//                       </button>
...
//     { id: "categorie", icon: Folder, label: "Catégories" },
//     { id: "produit", icon: Package, label: "Produits" },
//     { id: "ventes", icon: ShoppingCart, label: "Ventes" },
//     { id: "parametres", icon: Settings, label: "Paramètres" }
//   ];
...
//                   <Icon size={28} className={activePage === item.id ? "text-white" : "text-blue-300"} />
//                   <span className="text-xl font-semibold">{item.label}</span>
//                   {activePage === item.id && <div className="ml-auto w-2 h-2 bg-white rounded-full" />}
//                 </button>
//               );
//             })}
//           </div>
...
//                   >
//                     <div className="p-4 border-b border-gray-200">
//                       <p className="text-gray-800 font-semibold">{user?.nom || "Admin"}</p>
//                       <p className="text-gray-500 text-sm">{user?.email || "admin@example.com"}</p>
//                     </div>
//                     <div className="p-2">
//                       <button className="flex items-center gap-3 w-full p-3 text-gray-700 hover:text-blue-600 hover:bg-gray-50 rounded-lg transition-colors">
...
import { useState } from "react";
import { 
  AlertCircle, Folder, LayoutDashboard, LogOut, Menu, 
  Package, Settings, ShoppingCart, X, Bell, User, Search, Users 
} from "lucide-react"; 
import { motion, AnimatePresence } from "framer-motion";
import { useUser } from "../components/log/context/UserContext";
import { useNavigate } from "react-router-dom";
...
import AddProductForm from "./produits/AddProduits";
import Deconnexion from "../components/log/logout/Deconnexion";
import Carousel from "../home/pages/Carousel";
import AddUsers from "../components/log/admin/AddUsers";
...
export default function Home() {
  ...
  const handleLogout = async () => {
    await logout();
    navigate("/profil", { replace: true }); // 🔒 empêche retour arrière
  };
  ...
  const sidebarItems = [
    ...
    // { id: "parametres", icon: Settings, label: "Paramètres" },
    // { id: "Voir les pubs", icon: Settings, label: "Voir les pubs" }
  ];
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      >
        <nav className="h-full bg-gradient-to-b from-blue-900 to-blue-800 shadow-2xl border-r border-blue-700">
          ...
          <div className="absolute bottom-0 w-full p-4 border-t border-blue-700">
            <button
              onClick={handleLogout}
              className="flex items-center gap-4 w-full p-4 rounded-xl text-red-100 hover:bg-red-600/20 hover:text-white transition-all duration-300"
            >
              <LogOut size={28} className="text-red-300" />
              <span className="text-xl font-semibold">Déconnexion</span>
            </button>
      ...
      <div className="flex-1 flex flex-col lg:ml-80">
        ...
        <main className="flex-1 w-full min-h-screen overflow-y-auto bg-gray-50 p-6">
          ...
          {activePage === "categorie" && <Categorie />}
          {activePage === "ventes" && <AddVenteForm />}
          {activePage === "user" && <AddUsers />}
          {activePage === "logout" && <Deconnexion />}
          {activePage === "Voir les pubs" && <Carousel />}
          {activePage === "parametres" && <div>⚙ Module Paramètres</div>}
        </main>
      </div>
    </div>
