...
import { Eye, Edit, Trash2, Search, Filter, Download, Plus, ChevronDown, X, AlertCircle, Calendar, DollarSign, Package, BarChart3 } from "lucide-react";
...
const ListeProduits = () => {
  ...
  const contextActions = React.useMemo(() => {
    const handleDelete = () => {
      if (window.confirm(`Êtes-vous sûr de vouloir supprimer ${selectedRows.length} produits ?`)) {
        setToggleCleared(!toggleCleared);
        setProduits(produits.filter(p => !selectedRows.includes(p)));
      }
    };
